        ...
        <div class="card border-0 overflow-hidden">
            <div class="card-body">
                <div class="row mb-3 align-items-end">
                    ...
                    <div class="col-md-4 mb-3">
                        <label class="form-label fw-semibold" for="hospital">Company</label>
                        <select class="form-control choices" id="hospital" multiple>
                            <option value="">All</option>
                            @foreach ($hospitals as $hospital)
                                <option value="{{$hospital->id}}">{{$hospital->company_name}}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="table-responsive">
                    <table class="table w-100" id="rent_reportTable">
...
<script>
    $(document).ready(function () {
        ...
        $('#exportPdf').on('click', function () {
            let url = `{{ route('rent_report.pdf') }}`;
            if (applyFilter) {
                let from_date = $('#from_date').val();
                let to_date   = $('#to_date').val();
                let hospitals      = $('#hospital').val() ?? [];

                url += `?from_date=${from_date}&to_date=${to_date}&hospitals=${hospitals}`;
            }
            
            window.open(url, '_blank');
        });

        let table = $('#rent_reportTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                url: "{{ route('rent_report.ajax') }}",
                data: function (d) {
                    if (applyFilter) {
                        d.from_date = $('#from_date').val();
                        d.to_date   = $('#to_date').val();
                        d.hospitals      = $('#hospital').val() ?? [];
                    }else{
                        d.from_date = '';
                        d.to_date   = '';
                        d.hospitals     = [];
